...
HTTP 401凭证无效；429短时限流/繁忙若带retryable和retry_after，地点客户端按指定时间仅自动重试一次；日额度耗尽及其他错误停止，不连续重试；上游失败记录实际原因，不编造结果。服务不能访问时先报告，不把“有服务URL”当成查询过；按用户选择再用自有Key方式接力。小红书/点评仍由用户授权的浏览器读取。
...
先按城市＋完整地点名称去重，复用已核实坐标；只用service.py顺序查询，不直接并发curl、不用线程池或多Agent并行查同一批地点。客户端用本地SQLite锁串行化同一安装的地点请求，每次完成后间隔至少6秒；服务端对同城同名地点缓存1小时，缓存命中会返回gateway_cache.fetched_at，应保留原获取时间，不冒充新查询。未缓存地点由服务端串行发往高德，间隔至少1.1秒。

每成功一个地点就保存返回结果及已完成清单；中断后只补剩余地点，不重跑整批。限流等待由客户端处理（最多一次），仍失败时向用户说明错误和待补地点，停止本批；不通过改名、换凭证、删配置或分批快速重试规避限制。缓存只帮助稳定查询，不保证免除网关额度；高德上游报错不能一概当成临时限流自动重试。
